...
function procurauserinfo() {
    ...
    $(".bubble").show();
    $(".piechart").hide();
$("#crit").html(" LEGENDA: <br> <br> Compatibilidade entre as bandas mais ouvidas de "+username+" e dos seus amigos.");
}
...
function procurausermusicas() {
    ...
    $("#image").hide();
    $("#info").show();   
    $("#crit").html(" LEGENDA: <br> <br> Músicas mais reproduzidas por "+username+".");
}
...
function procurauserartistas() {
    ...
    $("#image").hide();
    $("#info").show();
    $("#crit").html(" LEGENDA: <br> <br> Artistas mais reproduzidos por "+username+".");
...
function getUserTopArtists() {
    ...
    $.get(base_url, data).done(processUserTopArtists).fail(logError("obter artistas do utilizador "+username));
}

function processUserTopArtists(data) {
    if($.isArray(data.topartists.artist)){ 
        for(var i=0; i < data.topartists.artist.length; i++){
            artistas[data.topartists.artist[i].name] = data.topartists.artist[i].playcount;
        }
    }
    else{
        artistas[data.topartists.artist.name] = data.topartists.artist.playcount;
    }
...
function getUserTopTracks()
{
    ...
    $.get(base_url, data).done(processUserTopTracks).fail(logError("obter musicas favoritas do utilizador "+ username));
}
...
function processUserTopTracks(data)
{
    ...
    setup: function(data, w, h){
        ...
        methods.height = h;
        methods.fill = d3.scale.ordinal()
        .range(["rgba(42, 156, 216, 0.96)", "rgba(129, 179, 183, 0.97)", "rgba(31, 78, 98, 0.94)", "rgba(41, 80, 167, 0.74)"])      
        var margin = methods.getMargin();   
        var selector = methods.el["selector"];  
        var svg = d3.select(selector)
            .append("svg")
...
var dataCharttoptracks = eval(jsontoptracks);
//alert(dataCharttoptracks);
//console.log(dataCharttoptracks);

var clone = jQuery.extend(true, {}, dataCharttoptracks);
//__invoke bubble
$('[data-role="bubble"]').each(function(index) {
    var selector = "bubble"+index;
    $(this).attr("id", selector);
